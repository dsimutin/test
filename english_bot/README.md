...
## Multi-tenant: разные слова разным ученикам

Бот сам ведёт служебный лист **`_students`** в твоей главной таблице.
Когда ученик пишет боту в первый раз — он **автоматически появляется в
этом листе** со своим Telegram ID и именем. Тебе ничего у него
спрашивать не надо.

Структура (бот сам создаст и заполняет):

| telegram_id | name      | words_tabs            | verbs_tabs        |
|-------------|-----------|-----------------------|-------------------|
| 352086154   | Дима      |                       |                   |
| 555111222   | Христина  | `Христина_урок1`      |                   |
| 777888999   | Иван      | `Иван_л1, Иван_л2`    | `irregular_verbs` |

- `telegram_id` и `name` — **бот заполняет сам**, как только ученик
  пишет боту в первый раз
- `words_tabs` — **ты** вписываешь имя(имена) вкладок в этой же таблице
  с его словами. Пусто → главная вкладка слов.
- `verbs_tabs` — то же для глаголов. Пусто → главная вкладка глаголов.
  Обычно глаголы у всех общие → оставляй пустым.
- Несколько вкладок — через запятую: `tab1, tab2`

**Дефолт для новых учеников:** обе колонки пустые → видит главные
вкладки. Не настраивал — не важно, бот работает.

**Полный flow для нового ученика:**
1. Даёшь ссылку на бота
2. Он жмёт `/start` → через несколько секунд появляется в `_students`
3. (Опционально) Создаёшь для него отдельные вкладки в той же таблице:
   `Аня_урок1`, `Аня_урок2`, ... (структура колонок такая же как у
   главной `vocabulary`/`Словарь`)
4. В `_students` в его строке пишешь имя вкладки в `words_tabs`
5. В течение 30 минут (или жмёшь 🔄) у него подгружается контент

**Открыть Урок 2:** в `words_tabs` дописываешь `, Аня_урок2`. Всё.

Всё в одной таблице — никаких лишних расшариваний.

**Личная таблица ученика** должна иметь ту же структуру что главная:
- Лист `vocabulary` (или `Словарь`) с колонками `word, translation, ... lesson`
- Лист `irregular_verbs` (или `Глаголы`) с колонками `infinitive, past_simple, past_participle, translation, ... lesson`

Команда **`/assignments`** показывает текущее распределение по ученикам.
