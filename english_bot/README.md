...
Если у тебя несколько учеников и у каждого свой набор слов / свой темп,
бот поддерживает индивидуальные назначения через служебный лист
**`_students`** в твоей главной таблице.

Структура (бот сам создаст лист с заголовком при первом старте):
...
- `telegram_id` — узнать через `/myid`
- `spreadsheet_id` — ID **личной таблицы ученика** (из URL между `/d/` и `/edit`).
  Пусто → читаем из главной таблицы (для теста).
- `active_lessons` — что открыто сейчас: `1`, `1,2`, `*` (всё), `Travel,Food` —
  любые значения которые встречаются в колонке `lesson` личной таблицы.

**Как работает:**
- Лист `_students` пуст → single-tenant: все ученики видят слова из главной таблицы.
- Появилась хоть одна строка → multi-tenant: **только** перечисленные ученики
  получают контент, причём из своих источников и только по своим урокам.

**Чтобы открыть Христине Урок 2:** в её строке меняешь `1` на `1,2`.
В течение 30 минут (или жмёшь 🔄) она увидит новые слова.

**Новый ученик:**
1. Расшарь его личную таблицу с service-account как **Editor**
2. Добавь строку в `_students`
3. Готово
