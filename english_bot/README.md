...
- `/myid` — узнать свой Telegram ID (работает у всех)
- `/students` — список учеников со сводкой
- `/student <telegram_id>` — детально по одному ученику:
  последние 5 сессий, слова с ошибками, глаголы с ошибками
  (с разбивкой Past Simple / Past Participle)

### Настройка

1. Напиши боту `/myid` — получишь свой ID.
2. В Railway → Variables добавь `TEACHER_IDS=<твой_id>` (можно несколько
   через запятую: `TEACHER_IDS=11111,22222`).
3. Redeploy. Теперь `/students` доступен только тебе.

Ученикам ничего не нужно делать — они просто пользуются ботом, ты
видишь их прогресс в реальном времени.
