...
|-------------|-----------|-----------------------|----------------|
| 352086154   | Дима      |                       | *              |
| 555111222   | Христина  | `1A2bCd...XyZ`        | 1              |
| 777888999   | Иван      | `9Z8yXw...AbC`        | 1,2            |

- `telegram_id` и `name` — **бот заполняет сам**, когда ученик впервые
  начинает с ним общаться
- `spreadsheet_id` — **ты** вписываешь ID личной таблицы ученика
  (берётся из URL между `/d/` и `/edit`). Пусто → читаем из главной таблицы.
- `active_lessons` — **ты** указываешь что открыто: `1`, `1,2`, `*` (всё),
  `Travel,Food` — любые значения из колонки `lesson` в его таблице.

**Дефолт для новых учеников:** `spreadsheet_id` пусто, `active_lessons = *`
→ ученик сразу видит все слова из главной таблицы. Не настраивал — не
важно, бот работает.

**Полный flow для нового ученика:**
1. Даёшь ему ссылку на бота
2. Он жмёт `/start` → автоматически появляется в `_students` через
   несколько секунд
3. (Опционально) Открываешь `_students`, видишь его строку, заполняешь
   `spreadsheet_id` и `active_lessons` если хочешь дать ему персональные
   слова. Перед этим — расшарь его личную таблицу с service-account
   как **Editor**.
4. В течение 30 минут (или жмёшь 🔄) у него обновится контент.

**Открыть Христине Урок 2:** в её строке меняешь `1` на `1,2`. Всё.
